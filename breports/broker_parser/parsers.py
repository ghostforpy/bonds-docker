...
class TinkoffParserXLS:
    ...
    def _get_money_movement(self):
        data = tinkoff_xls_scripts.get_data_by_section(
            self.sheet, self.SECTIONS_NAMES['section_money_movement']
        )
        return_decimal = tinkoff_xls_scripts.return_decimal_replase_comma_to_dot
        result = dict()
        for i in data:
            ...
            temp['outgoing_balance'] = return_decimal(i[2])
            temp['plan_outgoing_balance'] = return_decimal(i[3])
            temp['crediting'] = return_decimal(i[4])
            result[i[0]] = temp
        return result
